...
// netlify/functions/qa-data.js
var { neon } = require_dist();
var sql = neon();
var WORKSPACE_ID = "main";
async function ensureTable() {
  await sql`
    CREATE TABLE IF NOT EXISTS qaforge_state (
      id text PRIMARY KEY,
      data jsonb NOT NULL,
...
exports.handler = async (event) => {
  try {
    ...
    if (event.httpMethod === "GET") {
      const rows = await sql`SELECT data, updated_at FROM qaforge_state WHERE id = ${WORKSPACE_ID}`;
      if (!rows.length) {
    ...
    if (event.httpMethod === "POST") {
      ...
      if (!payload.data || typeof payload.data !== "object") {
        return { statusCode: 400, body: JSON.stringify({ error: "Missing data field" }) };
      }
      ...
      const rows = await sql`
        INSERT INTO qaforge_state (id, data, updated_at)
        VALUES (${WORKSPACE_ID}, ${JSON.stringify(payload.data)}::jsonb, now())
        ON CONFLICT (id) DO UPDATE SET data = EXCLUDED.data, updated_at = now()
        RETURNING updated_at
      `;
      return { statusCode: 200, body: JSON.stringify({ ok: true, updatedAt: rows[0].updated_at }) };
    }
